...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="background flex min-h-screen flex-col items-center"><div class="flex items-center"><h1 class="mb-4 text-center text-4xl font-extrabold md:text-5xl lg:text-6xl">Messages
    </h1></div>

  <div class="mx-auto max-w-3xl items-center">${
    messages
      ? `${each(messages, (message) => {
          return `<div class="message-list"><p>Message: ${escape(
            message.content
          )}</p>
          ...
          ${
            message.updatedAt
              ? `<p>Updated at: ${escape(message.updatedAt)}</p>`
              : ``
          }
          ${
            userId == message.sender.id
              ? `<div style="display: flex;"><button class="message-button">Edit Message</button>
              <form action="?/deleteMessage" method="POST"><button type="submit" class="message-button">Delete Message
                </button>
                <input type="hidden" id="messageId" name="messageId"${add_attribute(
                  "value",
                  message.id,
                  0
                )}>
                ...
                ? `${validate_component(Modal, "Modal").$$render(
                    ...
                    },
                    {
                      ...
                      default: () => {
                        ...
                        <textarea rows="10" cols="44" autofocus id="message" name="message" class="modal-text">${escape(
                          ""
                        )}</textarea>
...
//# sourceMappingURL=_page.svelte-1c613615.js.map
