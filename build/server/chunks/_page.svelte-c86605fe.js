...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${
    room && room.name
      ? `<a href="${"/chatrooms/" + escape(room.id, true)}">back</a>
<h1 class="mb-4 text-center text-4xl font-extrabold md:text-5xl lg:text-6xl">Edit Chatroom: ${escape(
...
<div style="display: flex; flex-direction: column; align-items: center; height: 100vh;"><div class="mb-2 text-2xl">Change Chatroom Name</div>
    <form action="?/rename" method="POST" class="flex" style="margin-bottom: 12px;"><input id="newName" name="newName" maxlength="15"${add_attribute(
      "value",
      room.name,
      0
    )}>
    ...
    <form action="?/exit" method="POST"><input type="hidden" name="room"${add_attribute(
      "value",
      room.id,
      0
    )}>
...
//# sourceMappingURL=_page.svelte-c86605fe.js.map
