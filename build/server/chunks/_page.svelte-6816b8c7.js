...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  <div class="mb-4"><div class="text-center text-lg font-medium mb-2">Friends:</div>
    ${
      friends
        ? `${each(friends, (friend) => {
            return `<div class="flex justify-between items-center mb-2"><div>${escape(
              friend.username
            )}</div>
          <div>${escape(friend.email)}</div>
          <form action="?/removeFriend" method="post" class="text-red-500"><input type="hidden" id="friendID" name="friendID"${add_attribute(
            "value",
            friend.id,
            0
          )}>
  ...
  <div><div class="text-center text-lg font-medium mb-2">Friend Requests:</div>
    ${
      friendRequests
        ? `${each(friendRequests, (friendRequest) => {
            return `<div class="flex items-center justify-center mb-2"><div class="px-5">${escape(
              friendRequest.from.email
            )}</div>
          <div>${escape(friendRequest.status)}</div>
          ${
            friendRequest.fromId != userId
              ? `<div class="flex gap-2"><form action="?/accept" method="POST"><input type="hidden" name="requestId"${add_attribute(
                  "value",
                  friendRequest.id,
                  0
                )}>
...
//# sourceMappingURL=_page.svelte-6816b8c7.js.map
